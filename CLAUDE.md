...
Trip Planner started as a Phase 1 Local Web MVP of a larger technical spec (trip creation, itinerary generation, itinerary viewing/editing), now with a Phase 2 slice layered on top: WhatsApp event reminders via Twilio (see "WhatsApp reminders" below) — pilot-scale, an in-process poller rather than the original spec's Redis/BullMQ queue. Stripe payments and the React Native mobile app are still **not implemented** — see `README.md` for the full "out of scope" list. Do not add those unless asked; the DB schema still reserves columns for Stripe (`stripe_customer_id`, `subscription_tier`) that go unused.
...
### WhatsApp reminders — in-process poller, no job queue yet
`src/services/whatsapp.ts` follows the same mock/real seam as the others (`mockWhatsAppService` logs instead of sending; `twilioWhatsAppService` uses the Twilio WhatsApp API, requiring `TWILIO_ACCOUNT_SID`/`TWILIO_API_KEY_SID`/`TWILIO_API_KEY_SECRET`/`TWILIO_WHATSAPP_FROM` — auth via an API Key + Secret paired with the Account SID it acts on behalf of, Twilio's recommended alternative to the primary Account SID + Auth Token), selected by the same `USE_MOCKS` flag. Unlike the other integrations, it isn't driven by trip generation — `src/services/notificationScheduler.ts#startNotificationScheduler()` (started from `src/index.ts` after `app.listen()`) polls every 60s for `ItineraryEvent`s with `notifyWhatsapp: true`, `whatsappNotifiedAt: null`, and a `startTime` within `WHATSAPP_REMINDER_MINUTES_BEFORE` minutes (default 60), sends a reminder to the trip owner's `User.phoneNumber`, and stamps `whatsappNotifiedAt`. The due-check itself is a pure function (`isReminderDue()`) kept separate from the Prisma query so it's unit-testable without a DB.

This is a deliberate pilot-scale choice, not the original spec's Redis/BullMQ design — it won't survive a backend restart mid-window and won't work correctly if the service is ever scaled to more than one instance (each instance would poll independently). Revisit with a real queue if either of those becomes a problem. `updateEvent()` in `trips.service.ts` resets `whatsappNotifiedAt` back to `null` whenever `notifyWhatsapp` is turned on or `startTime` changes, so re-opting-in or rescheduling gets a fresh reminder window rather than being silently skipped forever.

Against the Twilio **Sandbox** (the default until a WhatsApp Business Account is approved), a recipient must first join the sandbox by sending its join code to the sandbox's WhatsApp number from their own WhatsApp — Twilio will otherwise accept the send request but the message never arrives, with no error surfaced to this app.
