...
- **Arrival method (flight/train/car):** the trip form asks how you're arriving. Flight lets you optionally name a specific airport (a city can have several serving different airlines — e.g. Paris has CDG, Orly, and Ryanair's Beauvais); train produces a real named station instead of an airport (e.g. Gare du Nord, correct for a Eurostar arrival from London); car needs no station/airport at all. The same choice is reused for the return leg — a round trip overwhelmingly departs from the same airport/station it arrived at, and the LLM is explicitly told not to switch without reason (verified live: an unspecified case previously drifted from Gare du Nord on arrival to Gare de Lyon on departure).
